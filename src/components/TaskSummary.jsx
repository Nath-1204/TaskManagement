...
const TaskSummary = () => {
  ...
  useEffect(() => {
    if (currentWorkspace) {
      setTasks(currentWorkspace.projects.flatMap(
    ...
    <div className='space-y-6'>
      {
        summaryCards.map((card) => (
          ...
          dark:to-zinc-900/50 border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 
          dark:border-zinc-700 transition-all duration-200 rounded-lg overflow-hidden'
          >
            <div className='border-b border-zinc-200 dark:border-zinc-800 p-4 pb-3'>
              <div className='flex items-center gap-3'>
                <div className='p-2 bg-zinc-50 dark:bg-zinc-800 rounded-lg'>
                  <card.icon className='w-4 h-4 text-gray-500 dark:bg-zinc-600 rounded-lg'/>
                </div>
                <div className="flex items-center justify-between flex-1">
                  <h3 className='text-sm font-medium text-gray-800 dark:text-white'>
                    {card.title}
                  </h3>
                  <span className={`inline-block mt-1 p-2 py-1 rounded text-xs font-semibold ${card.color}`}>
                    {card.count}
                  </span>
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    {
                      card.items.map((issue) => (
                        <div 
                          key={issue.id} 
                          className="p-3 rounded-lg bg-zinc-50 dark:bg-zinc-900 hover:bg-zinc-100 
                        ...
                        >
                          ...
                          <p className="text-xs text-gray-600 dark:text-zinc-400 capitalize mt-1">
                            {issue.type} • {issue.priority} priorité
                          </p>
                        </div>
                      ))
                    }
                                
                    {
                      card.count > 3 && (
                        <button 
                          className="flex items-center justify-center w-full text-sm text-gray-500 
                          dark:text-zinc-400 hover:text-gray-800 dark:hover:text-white mt-2"
                        >
                          Voir {card.count - 3} plus <ArrowRight className="w-3 h-3 ml-2" />
                        </button>
                      )
                    }
                  </div>
