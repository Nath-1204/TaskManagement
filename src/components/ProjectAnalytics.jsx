import { AlertTriangle, ArrowRightIcon, CheckCircle, Clock, Users } from 'lucide-react';
...
const ProjectAnalytics = ({ project, tasks }) => {
  ...
  const { stats, statusData, typeData, priorityData } = useMemo(() => {
    ...
    tasks.forEach((task) => {
      ...
      if (new Date(task.due_date) < now && task.status !== "DONE") stats.overdue++;

      if (statusMap[task.status] !== undefined) statusMap[task.status]++;
      if (typeMap[task.type] !== undefined) typeMap[task.type]++;
      if (priorityMap[task.priority] !== undefined) priorityMap[task.priority]++;
    });
        ...
        <div className="space-y-4">
          {
            priorityData.map((p) => (
              <div className="space-y-2">
                ...
                <div className="w-full bg-zinc-300 dark:bg-zinc-800 riunded-ull h-1.5">
                  <div
                    className={`h-1.5 rounded-ull ${PRIORITY_COLORS[p.name]} `}
