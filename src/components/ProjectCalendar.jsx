import { addMonths, eachDayOfInterval, endOfMonth, format, isBefore, isSameDay, startOfMonth, subMonths } from 'date-fns';
...
const typeColors = {
  ...
  IMPROVEMENT: "bg-purple-200 text-purple-800 dark:bg-purle-500 dark:text-purple-900",
...
const ProjectCalendar = ({ tasks }) => {
  ...
  const upcomingTasks = tasks
    .filter((task) => task.due_date && !isBefore(task.due_date, today))
  ...
  return (
    <div className='grid lg:grid-cols-3 gap-6'>
      ...
      <div className="lg:col-span-2">
        <div className="not-dark:bg-white dark:bg-gradient-to-br dark:from-zinc-800/70 
          dark:to-zinc-900/50 border bordre-zinc-300 dark:border-zinc-800 rounded-lg p-6 "
        >
          <div className="flex items-center justify-between mb-4 ">
            <h2 className="text-zinc-900 dark:text-white text-md flex gap-2items-center max-sm:hidden">
          ...
          <div className="grid grid-cols-7 gap-2">
            {daysInMonth.map((day) => {
              ...
              const hasOverdue = dayTasks.some((t) => t.status !== "DONE" && isBefore(t.due_date));
              ...
              return (
                <button
                  ...
                  className={`sm:h-14 rounded-md flex flex-col items-center justify-center text-sm
                    ${isSelected ? "bg-blue-200 text-blue-900 dark:bg-blue-600 dark:text-white" : "bg-zinc-50 text-zinc-900 dark:bg-zinc-800/40 dark:text-zinc-300 hover:bg-zinc-100 dark:hiver:bg-zinc-700"}
          ...
          >
            ...
            <div className="space-y-3">
              {getTasksForDate(selectedDate).map((task) => (
                <div
                  key={task.id}
                  className={`bg-zinc-50 dark:bg-zinc-800/40 hover:bg-zinc-100 dark:hiver:bg-zinc-800 
                  trasnition p-4 rounded border-l-4 ${priorityBorders[task.priority]}`}
                >
                  ...
                  <div className="flex justiy-between text-xs text-zinc-600 dark:text-zinc-400">
      ...
      <div className="space-y-6">
        {/* Upcoming Tasks */}
        <div className="bg-white dark:bg-zinc-950 dark:bg-gradient-to-br dark:from-zinc-800/70 
        ...
        >
          ...
          {upcomingTasks.length === 0 ? (
            <p className='text-zinc-500 dark:text-zinc-400 text-sm text-center'>No upcomming tasks</p>
          ) : (
        ...
        {overdueTasks.length > 0 && (
          <div className='bg-white dark:bg-zinc-950 border border-red-300 dark:border-red-500 border-l-4 rounded-lg p-6'>
            ...
            <div className="space-y-2">
              {
                overdueTasks.slice(0, 5).map((task) => (
                  <div
                    key={task.id}
                    className='bg-red-50 dark:bg-red-900/20 hover:bg-red-600'
                  >
