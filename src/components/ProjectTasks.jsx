import { Bug, Calendar, CalendarIcon, GitCommit, MessageSquare, Square, Trash, Zap } from 'lucide-react';
...
const ProjectTasks = ({ tasks }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const confirm = window.confirm("Es-tu sûr de vouloir supprimer la tâche selectionné ? ");

      if (!confirm) return;

      toast.loading("Suppression tâche......");
      ...
      toast.dismissAll();
      toast.success('Tâche supprimé avec succes')
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-4 mb-4">
        ...
        { (filters.status || filters.type || filters.priority || filters.assignee ) && (
          ...
          >
            <XIcon className="size-3"/> Réinitialiser
          </button>
        )}

        {
          selectedTasks.length > 0 && (
            <button
              ...
              className='px-3 py-1 flex items-center gap-2 rounded bg-gradient-to-br from-purple-400 to-indigo-500 text-zinc-100 dark:text-zinc-200 text-sm transition-colors'
            >
      ...
      {/* Tasks table */}
      <div className="overflow-auto rounded-lg lg:border border-zinc-300 dark:border-zinc-">
              ...
              <thead className="text-xs uppercase dark:bg-zinc-800/70 text-zinc-500 dark:text-zinc-400">
                <tr>
                  <th className="pl-2 pr-1">
                    <input 
                      type="checkbox" 
                      onChange={() => selectedTasks.length > 1 
                        ? setSelectedTasks([]) 
            ...
            {filteredTasks.length > 0 ? (
              filteredTasks.map((task) => {
                ...
                return (
                  <div 
                    key={task.id}
                    className='dark:bg-gradient-to-be dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4 flex flex-col gap-2'
                  >
