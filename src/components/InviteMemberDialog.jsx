...
const InviteMemberDialog = ({ isDialogOpen, setIsDialogOpen }) => {
  ...
  const currentWorskspace = useSelector((state) => state.workspace?.currentWorskspace);
      ...
      <div className="bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-xl p-6 w-full max-w-md text-zinc-900 dark:text-zinc-200">
        ...
        <div className="mb-4">
          <h2 className="text-xl font-bold flex items-center gap-2">
            ...
            Invite un membre d'équipe
          </h2>
          {
            currentWorskspace && (
              <p className="text-sm text-zinc-700 dark:text-zinc-400">
                Invitez vers le lieu de travail: <span className="text-blue-600 dark:text-blue-400 ">{currentWorskspace.name}</span>
              </p>
            )
          }
        </div>
        ...
        <form onSubmit={handleSubmit} className='space-y-4'>
          ...
          <div className="space-y-2">
            <label htmlFor="email" className='text-sm font-medium textzinc-900 dark:text-zinc-200'>
              Email Address
            </label>
            <div className="relative">
              ...
              <input 
                ...
                className='pl-10 mt-1 w-full rounded border border-zinc-300 dark:border-zinc-700 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-200
                text-sm placeholder-zinc-400 dark:placeholder-zinc-500 py-2 focus:outline-none focus:border-blue-500'
                required 
              />
            </div>
          </div>
          ...
          <div className='flex justify-end gap-3 pt-2'>
            ...
            <button 
              type="submit"
              disabled={isSubmitting || !currentWorskspace}
            ...
            >
              {isSubmitting ? "Envoie....." : "Envoie de l'invitation"}
            </button>
          </div>
